...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    /* TODO: YOUR CODE HERE */
    PyObject *argmat = NULL;
    if (PyArg_Parse(args, "O", &argmat)) {
        if (!PyObject_TypeCheck(argmat, &Matrix61cType)) {
            PyErr_SetString(PyExc_TypeError, "Argument must of type numc.Matrix!");
            return NULL;
        }
        Matrix61c* argmat2 = (Matrix61c *) argmat;
        if (argmat2->mat->rows != self->mat->rows || argmat2->mat->cols != self->mat->cols) {
        ...
        int fail = add_matrix(result, self->mat, argmat2->mat);
        if (fail) {
...
static PyObject *Matrix61c_sub(Matrix61c* self, PyObject* args) {
    ...
    if (PyArg_Parse(args, "O", &argmat)) {
        ...
        int fail = sub_matrix(result, self->mat, argmat2->mat);
        if (fail) {
          deallocate_matrix(result);
          PyErr_SetString(PyExc_TypeError, "Invalid Dimensions!");
          return NULL;
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
    ...
    if (PyArg_Parse(args, "O", &argmat)) {
        ...
        Matrix61c* argmat2 = (Matrix61c *) argmat;
        if (argmat2->mat->rows != self->mat->cols) {
          PyErr_SetString(PyExc_TypeError, "Invalid Dimensions!");
          return NULL;
        }
        Matrix61c* rv = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
        matrix *result = NULL;
        int alloc_failed = allocate_matrix(&result, self->mat->rows, argmat2->mat->cols);
        if (alloc_failed) {
          PyErr_SetString(PyExc_RuntimeError, "Invalid Dimensions!");
          return NULL;
        }
        int fail = mul_matrix(result, self->mat, argmat2->mat);
        if (fail) {
          deallocate_matrix(result);
          PyErr_SetString(PyExc_TypeError, "Invalid Dimensions!");
          return NULL;
        ...
        rv->mat = result;
        rv->shape =  PyTuple_Pack(2, PyLong_FromLong(self->mat->rows), PyLong_FromLong(argmat2->mat->cols));
        return (PyObject*) rv;
    }
    return NULL;
}
...
static PyObject *Matrix61c_neg(Matrix61c* self) {
    ...
    int fail = neg_matrix(result, self->mat);
    if (fail) {
      deallocate_matrix(result);
      PyErr_SetString(PyExc_TypeError, "Invalid Dimensions!");
      return NULL;
...
static PyObject *Matrix61c_abs(Matrix61c *self) {
    ...
    int fail = abs_matrix(result, self->mat);
    if (fail) {
      deallocate_matrix(result);
      PyErr_SetString(PyExc_TypeError, "Invalid Dimensions!");
      return NULL;
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    ...
    Matrix61c* rv = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    int fail = pow_matrix(result, self->mat, power);
    if (fail) {
      deallocate_matrix(result);
      PyErr_SetString(PyExc_TypeError, "Invalid Arguments");
      return NULL;
